...
function Checkout(props) {
    ...
    useEffect(() => {
        ...
        if (check_action) {
            Cart.getCartsByUser({ user_id: user_id })
                .then(r => {
    ...
    const [from, set_from] = useState('470 Trần Đại Nghĩa, Hòa Quý, Ngũ hành Sơn, Đà nẵng')
                                        ...
                                        <table className="table">
                                            ...
                                            <tbody>
                                                {
                                                    carts && carts.map(value => (
                                                        <tr className="cart_item" key={value._id}>
                                                            <td className="cart-product-name">{value.dish_name}<strong className="product-quantity"> × {value.quantity}</strong></td>
                                                            <td className="cart-product-total"><span className="amount">{new Intl.NumberFormat('vi-VN', { style: 'decimal', decimal: 'VND' }).format(parseInt(value.price) * parseInt(value.quantity)) + ' VNĐ'}</span></td>
                                                        </tr>
                                                    ))
                                                }
                                            </tbody>
                                            <tfoot>
                                                ...
                                                <tr className="order-total">
                                                    <th>Order Total</th>
                                                    <td><strong><span className="amount">{new Intl.NumberFormat('vi-VN', { style: 'decimal', decimal: 'VND' }).format(parseInt(total_price) + parseInt(price)) + ' VNĐ'}</span></strong></td>
                                                </tr>
                                            </tfoot>
                                        </table>
                                    </div>
